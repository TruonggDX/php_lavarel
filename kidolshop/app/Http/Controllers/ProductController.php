...
class ProductController extends Controller
{
    ...
    public function show_product_details($ProductSlug) {
        ...
        if (!$this_pro || $this_pro->StatusPro == '0') {
            return Redirect::to('home')->send();
        }

        // Kiểm tra idCustomer, nếu không đăng nhập thì để null
        $hasPurchased = false;
        $idCustomer = Session::has('idCustomer') ? (int) Session::get('idCustomer') : null;
        if (Session::has('idCustomer')) {
            $idCustomer = Session::get('idCustomer');

            $hasPurchased = DB::table('bill') // Thay orders bằng bill
            ->join('billinfo', 'bill.idBill', '=', 'billinfo.idBill') // Thay order_details bằng billinfo
            ->where('bill.idCustomer', $idCustomer)
                ->where('billinfo.idProduct', $this_pro->idProduct)
                ->where('bill.Status', 2) // Giả sử 1 là trạng thái hoàn thành
                ->exists();
        }

        // Chỉ xử lý Viewer nếu người dùng đã đăng nhập và có idProduct hợp lệ
        if ($idCustomer !== null && $this_pro->idProduct) {
            $existingView = Viewer::where('idCustomer', $idCustomer)
                ->where('idProduct', $this_pro->idProduct)
                ->exists();

            if (!$existingView) {
                // Giới hạn lịch sử xem sản phẩm của mỗi user là 3
                if (Viewer::where('idCustomer', $idCustomer)->count() >= 3) {
                    Viewer::where('idCustomer', $idCustomer)
                        ->orderBy('idView', 'asc')
                        ->first()
                        ->delete();
                }
        ...
        $list_pd_attr = ProductAttriBute::join('attribute_value', 'attribute_value.idAttrValue', '=', 'product_attribute.idAttrValue')
            ->join('attribute', 'attribute.idAttribute', '=', 'attribute_value.idAttribute')
            ->where('product_attribute.idProduct', $this_pro->idProduct)
            ->get();

        $name_attribute = $list_pd_attr->first();
        ...
        return view("shop.product.shop-single")->with(compact(
            'list_category',
            'list_brand',
            'product',
            'list_pd_attr',
