...
{
    /* ---------- Admin ---------- */
    public function update_status(Request $request, $idBill) {
        ...
        $statusNew = $request->input('status'); // Lấy trạng thái mới từ request

        if ($bill->Status == $statusNew) {
            return response()->json(['message' => 'Trạng thái không thay đổi'], 200);
        }

        // Cập nhật trạng thái mới vào bảng `bill`
        $bill->Status = $statusNew;
        $bill->save();

        // Lưu lịch sử cập nhật vào bảng `billhistory`
        DB::table('billhistory')->insert([
            'idBill' => $idBill,
            'AdminName' => Session::get('AdminName'), // Lấy tên admin từ session
            'Status' => $statusNew,
            'created_at' => now(),
            'updated_at' => now(),
        ]);
        ...
        public function bill_info($idBill){
            ...
            $list_bill_info = BillInfo::join('product', 'product.idProduct', '=', 'billinfo.idProduct')
                ...
                ->join('bill', 'bill.idBill', '=', 'billinfo.idBill') // Thêm join bảng bill
                ->where('billinfo.idBill', $idBill)
                ->select('product.ProductName', 'product.idProduct', 'productimage.ImageName', 'billinfo.*', 'bill.Status') // Lấy thêm Status
                ->get();
