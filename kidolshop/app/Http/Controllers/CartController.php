...
class CartController extends Controller
{
    ...
    public function show_cart(){
        ...
        $recommend_pds_array = []; // Khởi tạo mảng
        $checked_pro = []; // Khởi tạo mảng chứa ID sản phẩm đã xét
        ...
        foreach ($list_pd_cart as $key => $pd_cart) {
            ...
            // Khởi tạo mảng rỗng trước khi thêm dữ liệu để tránh lỗi
            $recommend_pds_array[$key] = [];

            $list_recommend_pds = Product::whereRaw("MATCH (ProductName) AGAINST (?)", Product::fullTextWildcards($pd_cart->ProductName))
                ->whereNotIn('idProduct', [$pd_cart->idProduct])
                ->where('StatusPro', '1')
                ->select('idProduct');

            $list_recommend_pds->where(function ($query) use ($idBrand, $idCategory) {
        ...
        // Gộp tất cả sản phẩm gợi ý từ nhiều mảng con
        $recommend_pds_arrays = array_merge(...array_values($recommend_pds_array));

        // Loại bỏ trùng lặp và sản phẩm đã có trong giỏ
        $recommend_pds_unique = array_unique(array_diff($recommend_pds_arrays, $checked_pro));
    ...
    public function submit_payment(Request $request){
        ...
        if($data['checkout'] == 'vnpay'){
            ...
            $vnp_Returnurl = "http://127.0.0.1:8000/success-order";
