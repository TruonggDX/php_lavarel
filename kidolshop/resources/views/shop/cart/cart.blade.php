                ...
                @if(!empty($id_pds))
                    <div class="row">
                        @foreach($id_pds as $id_pd)
                                <?php $product = App\Http\Controllers\CartController::get_product($id_pd); ?>
                            @if($product) {{-- Kiểm tra nếu product tồn tại --}}
                            <div class="col-lg-3 col-sm-6">
                                <div class="single-product">
                                    <div class="product-image">
                                            <?php
                                            $images = json_decode($product->ImageName, true); // Giải mã JSON thành mảng
                                            $image = !empty($images) ? $images[0] : 'default.jpg'; // Lấy ảnh đầu tiên hoặc ảnh mặc định
                                            ?>
                                        <a href="{{ URL::to('/shop-single/'.$product->ProductSlug) }}">
                                            <img src="{{ asset('storage/kidoldash/images/product/'.$image) }}" alt="{{ $product->ProductName }}">
                                        </a>
                                    </div>
                                    <div class="product-content text-center">
                                        <h4 class="product-name">
                                            <a href="{{ URL::to('/shop-single/'.$product->ProductSlug) }}">{{ $product->ProductName }}</a>
                                        </h4>
                                    </div>
                                </div>
                            </div>
